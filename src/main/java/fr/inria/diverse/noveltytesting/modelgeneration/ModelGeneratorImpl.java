...
import fr.inria.diverse.noveltytesting.coverage.Coverage;
import fr.inria.diverse.noveltytesting.coverage.CoverageImpl;
...
public class ModelGeneratorImpl implements ModelGenerator {
    ...
    private Coverage cov = new CoverageImpl();
    ...
    public Interface generateModel(Class<?> clazz,String sourcePackage) throws InstantiationException, IllegalAccessException, IOException {
    	List<ImplementedClass> Classes= new LinkedList<ImplementedClass>();

    	List<Class<?>>  myClasses= new LinkedList<Class<?>>();
        Interface i = new Interface();
        
        i.setName(clazz.getName());
        myClasses= cf.findClassesPerInterface(clazz, sourcePackage);

        for (Class<?> clas : myClasses) {
        	ImplementedClass ic= new ImplementedClass();
        	ic.setClasses(clas);

            
            List<Constructor> constructorz = new ArrayList<>();
            for (Constructor<?> c : clas.getDeclaredConstructors()) {
                if (exclusionPattern == null) {
                	constructorz.add(c);
                } else {
                    if (!c.getName().startsWith(exclusionPattern)) {
                    	constructorz.add(c);
                    }
                }
            }

            for (java.lang.reflect.Constructor<?> c : constructorz) {
            	fr.inria.diverse.noveltytesting.model.Method con = new fr.inria.diverse.noveltytesting.model.Method();
                ...
                for (java.lang.reflect.Parameter p : c.getParameters()) {
                    ...
                    param.setClassType(p.getType());
                    con.addParameter(param);

                }
               
                ic.addConstructor(con);
            }
        	
        	
        i.addImplementedClass(ic);
    ...
    public void generateData(Interface anInterface) {
        ...
        for(ImplementedClass ic :anInterface.getClasses()){
        	ic.getConstructors().forEach(this.generator::generateData);
        }
    }

    @Override
    public void executeMethods(String sourcePackage, Interface anInterface,String myCompiledClasses)
            throws IOException, Exception {
        try {
   
            cov.exec(sourcePackage, anInterface, myCompiledClasses);
